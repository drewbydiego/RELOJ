...
contenedorCuadroBotones.innerHTML = '<center><input type="text"class="cuadroRecordatorio" id="cuadroRecor" value="Escribe un recordatorio" onClick="cambioTextoAlarma()"><br><input type="checkbox" class="check" onClick="validacionCheck()"><span class="textoCheck">añadir mensaje</span><br><button id="est" class="btn" onClick="iniciarTemporizadorA()">Iniciar</button><button id="el" class="btn2" onClick="ResetearTemporizadorA()">Borrar</button><br></br> </center>'
...
function iniciarTemporizadorA(){
    //initialize the variable
    //var asignarNumeros_tempo = false;
    horamostrarA = hA.value;
    minutoMostrarA = mA.value;
    segundoMostrarA = sA.value;
    //asignarNumeros_tempo = true;
...
var horaCero = false;
var minutoCero = false;
var segundoCero = false;
    ...
    if(hA.value == 0 && mA.value == 0 && sA.value == 0){
        ...
        sA.value--;
        if(sA.value <= 0 && mA.value<=0 && hA.value<=0)
        {
            clearInterval(startTimerA);
            if(validarCheck == true){
                    if(msRecordatorio.value == "Escribe un recordatorio" || msRecordatorio.value == ""){
                        notificarVacio();
                        alert('No escribiste un recordatorio');
                    }else{
                        notificar();
                        alert(msRecordatorio.value);
                    }
...
function notificar() {
    if(Notification.permission !== "granted"){
        ...
        var notificaciones = new Notification(`¡TIEMPO FINALIZADO!`,
        {
            body: msRecordatorio.value
        });

        notificaciones.onclick = ()=>{
            alert(`-${msRecordatorio.value}-\n Tiempo establecido: \n ${horamostrarA}h ${minutoMostrarA}m ${segundoMostrarA}s`);
        }
    }
}
function notificarVacio() {
    if(Notification.permission !== "granted"){
        ...
        var notificaciones = new Notification(`¡TIEMPO FINALIZADO!`,
        {
            body: "No escribiste un recordatorio"
        });

        notificaciones.onclick = ()=>{
            alert(`Tiempo establecido: \n ${horamostrarA}h ${minutoMostrarA}m ${segundoMostrarA}s`);
        }
    }
}
